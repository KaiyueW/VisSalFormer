...
class ChartQAKNNRetriever:
    ...
    Stage 1 (image): retrieve the top `image_topk` most image-similar training
                       examples using a CLIP image encoder.
    Stage 2 (text):    from those candidates, re-rank by question-text similarity
                       (SentenceTransformer) and keep the top `num_examples`.
    """
    ...
    def _precompute_image_features(self):
        """Encode every training image and cache the normalized feature matrix [N, D]."""
        features = []
        loader = DataLoader(
            self.dataset,
        ...
        with torch.no_grad():
            for batch in tqdm(loader, desc="Precomputing training image features"):
                imgs = torch.stack(
                    [self.image_processor(img) for img in batch["image"]]
                ).to(self.device)
                feats = self.image_model.encode_image(imgs)
    ...
    def retrieve(self, test_image, test_query, num_examples, do_reverse=False):
        ...
        with torch.no_grad():
            ...
            img_input = self.image_processor(test_image).unsqueeze(0).to(self.device)
            query_img_feat = self.image_model.encode_image(img_input)
            query_img_feat = query_img_feat / query_img_feat.norm(dim=-1, keepdim=True)
            query_img_feat = query_img_feat.detach().cpu()  # [1, D]
            ...
            query_text_feat = self.text_model.encode(
                [test_query], convert_to_tensor=True, show_progress_bar=False
            )
            query_text_feat = query_text_feat / query_text_feat.norm(dim=-1, keepdim=True)

            cand_texts = [c["query"] for c in candidates]
            cand_text_feats = self.text_model.encode(
                cand_texts, convert_to_tensor=True, show_progress_bar=False
            )
            cand_text_feats =  cand_text_feats / cand_text_feats.norm(dim=-1, keepdim=True)
...
def run(
    ...
    if save_image_features is not None:
        ...
        retriever.save_image_features(save_image_features)
