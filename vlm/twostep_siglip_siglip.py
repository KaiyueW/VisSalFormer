...
class ChartQAKNNRetriever:
    ...
    Stage 2 (text):    from those candidates, re-rank by question-text similarity
                       using the SigLIP text encoder and keep the top `num_examples`.
    ...
    def retrieve(self, test_image, test_query, num_examples, do_reverse=False):
        with torch.no_grad():
            ...
            query_text_feat = self._encode_texts_siglip([test_query])

            cand_text_feats = self.text_features[top_indices]
...
if __name__ == "__main__":
    ...
    run(
        ...
        output_json="output/siglip_siglip_knn_fewshot_examples.json",
        num_shots=8,
        image_topk=200,
